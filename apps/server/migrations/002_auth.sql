-- generated by `@better-auth/cli`, which turned out to be a version behind the
-- library and produced a schema missing account.issuer. 004 repairs it, and
-- src/db/gen-auth-schema.ts is the generator to use from now on: it reads
-- better-auth's own definitions, so it cannot skew from the runtime.
--
-- kept as-is, being the record of what was actually applied. never hand-edit an
-- applied migration; add a new one.
--
-- `organization` here is what we mean by a workspace. migration 003 repoints
-- films and assets at it and drops the placeholder table 001 created.
