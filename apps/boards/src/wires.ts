/**
 * Orthogonal connectors in the style of a flow diagram: leave horizontally,
 * turn once at a shared middle column, arrive horizontally. Corners are arcs
 * rather than mitres, which is what stops a dense fan-out reading as a grid.
 */
export function elbow(
  ...
  const midX = (x1 + x2) / 2
  const dy = y2 - y1
  // a straight run needs no corners at all
...
/** the small tab a wire attaches to on a card edge */
